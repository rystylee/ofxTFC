...
void ofApp::setup()
{
	...
	inputImg.allocate(64, 100, OF_IMAGE_GRAYSCALE);
	mInputImg.push_back(inputImg);

	ofFloatImage  outputImg;
	outputImg.allocate(64, 64, OF_IMAGE_COLOR);
...
void ofApp::update()
{
	...
	updateZ();
	if (ofGetFrameNum() % 30 == 0) randomizeInput();
}
...
void ofApp::updateZ()
{
	for (int batch = 0; batch < mInputDims[0]; batch++)
	{
		for (int z = 0; z < mInputDims[1]; z++)
		{
			...
			mZ[index] += (mTargetZ[index] - mZ[index]) * 0.1;
		}
	}
